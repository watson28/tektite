...
func createIterator(t *testing.T, lm *LevelManager, keyStart []byte, keyEnd []byte) *iteration.MergingIterator {
	...
	require.NoError(t, err)
	var chainIters []iteration.SimplerIterator
	for _, notids := range otids {
		...
		chainIter := iteration.NewChainingIteratorFromSimple(iters)
		chainIters = append(chainIters, chainIter)
	}
	mi, err := iteration.NewMergingIteratorFromSimple(chainIters, false, math.MaxUint64)
	require.NoError(t, err)
	return mi
}
